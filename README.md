...
Dreadstone Animation Forge is a proprietary Blender add-on for animation, protected damage-segment and stump authoring, trauma-field deformation authoring, and GLB/manifest export. This repository is the authoritative development source for add-on version **3.10.1**. Blender runtime acceptance targets **Blender 5.1.2**; repository static checks do not execute Blender APIs. The deformation-authoring build is `2026-07-18.trauma-hotfix.1`.

The v3.10.1 acceptance hotfix preserves the v3.10.0 Trauma Field workflow while adding safe legacy pair resynchronization, GLB split-seam-aware patch connectivity and geodesics, and viewport interoperability with Damage Authoring. Strict exact-index validation and non-destructive topology remain unchanged.
...
| Deformation Authoring | `3.10.1` |
| Deformation build | `2026-07-18.trauma-hotfix.1` |

## Install a release

1. Download `Dreadstone_Animation_Forge_v3_10_1.zip` from the GitHub Actions development artifact (or an approved release location when one is published).
...
An accepted v3.9.1 file with `DSB_ATTACHED_HEAD` and `DSB_SEGMENT_HEAD` but no registry is migrated additively to region `head` when its topology contract is valid. Existing paired standard keys are safety-checked during migration. When the attached key is finite and within its declared displacement maximum, a stale detached delta is rebuilt at the same vertex index through world space and its value driver is restored. The attached side is authoritative only for this exact Forge-managed legacy contract: missing legacy keys are not recreated automatically, arbitrary keys are not adopted, and unrepairable attached keys are not overwritten or deleted.

**REPAIR LEGACY PAIR SYNC** applies the same guarded operation to every Forge-managed legacy key without a procedural stamp stack in the active region. Healthy pairs remain unchanged except for restoring their detached value driver; procedural, missing, and unsafe pairs remain unchanged. The operator records `PASS`, `REPAIRED`, or `UNREPAIRABLE` sync metadata and then runs the unchanged strict deformation validation.
...
- **Selected Face Patch** captures one virtual-edge-connected face component, its raw face and vertex indices, area-weighted center/normal, world bounds, radius, selection hash, topology fingerprint, virtual-weld tolerance/digest, and virtual component count. Two faces connect only when they share a complete virtualized edge; corner-only contact remains disconnected.
...
Captures are rejected when their region, object, topology, indices, selection, or recorded virtual-weld state is stale. The rule is intentionally narrow: virtual welding is analytical only. The tolerance is `max(1e-7, world_bounds_diagonal * 1e-7)`, only coincident raw vertices within that tolerance share a virtual ID, and no Blender mesh merge operation occurs. **Surface Distance** keeps real mesh edges at their world-space lengths and adds deterministic zero-cost links between members of each virtual weld group before radius-limited Dijkstra traversal. This crosses legitimate imported GLB split seams without creating a world-space shortcut through nearby surfaces. Cached traversal is keyed by topology, object identity, selection, mode, range, weld digest, and resolved tolerance. **World Distance** remains available for compatibility and diagnosis.
...
The six v3.10.1 families are exactly:
...
The **Attached**, **Detached**, and **Both** inspection buttons clear viewport blocking only for the active registered pair and its generated DSB collection path, then apply the requested per-object viewport state. They report the blocking object or collection if the requested member still cannot be shown. This viewport inspection does not rewrite render/export visibility, `dsb_default_visible`, caps, sockets, other detached segments, or Damage Authoring preview contracts.
...
Version 3.10.1 intentionally does not include gore reveal, gore/slice decals, tissue exposure, puncture stamps, reveal masks, seam variants, arbitrary runtime cutting, creature-profile automation, topology-family generation, armor authoring, or game-runtime integration/morph blending.
...
The build writes `dist/Dreadstone_Animation_Forge_v3_10_1.zip`. It validates first, fixes archive ordering and timestamps, checks the exact installable layout (including `trauma_field.py`), and tests ZIP integrity. Repeated builds from identical source are byte-for-byte deterministic.
