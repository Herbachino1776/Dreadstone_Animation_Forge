...
3. **Run Blender runtime acceptance.** Complete and record the current procedures and recipes in `docs/USER_WORKFLOW_GUIDE.md`. For Forge 3.18, run the gore and core/compound acceptance scripts from appropriate prepared authoring `.blend` files. Record head left/front, body, both forearms, paired/core/compound results; cavity layer ordering, proudness, separation, depth, skinning, triangle counts; legacy modes; all identities/macros/seeds/scales; guard validation; proof renders; GLB export; and clean reimport. Static CI is not runtime acceptance or visual approval.
...
- [ ] Confirm the GLB/manifest satisfies `docs/RAISED_GORE_EXPORT_CONTRACT.md`: stable paired/core cavity or legacy nodes, mode-correct glTF-safe materials, layer/depth/skinning metrics, inactive defaults, deformation activation mapping, no preview-only nodes, geometry digests, and clean reimport.
